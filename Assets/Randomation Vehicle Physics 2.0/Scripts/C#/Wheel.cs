    ...
    void Start()
	{
		...
		if (tr.childCount > 0)
		{
			...
			if (canDetach)
			{
				...
				detachedWheel.layer = LayerMask.NameToLayer("Default");
...
//					sphereColNew.layer = GlobalControl.ignoreWheelCastLayer;
	...
	void Update()
	{
		...
		else
		{
			...
			if (rimMat)
			{
				if (rimMat.HasProperty("_Emission"))
				{
					//Make the rim glow
					...
					rimMat.SetColor("_Emission", popped ? Color.Lerp(Color.black, glowColor, glowAmount * rimGlow) : Color.black);
				}
			}
		}
	}
	...
	void RotateWheel()
	{
		if (tr && suspensionParent)
		{
			...
			tr.localEulerAngles = new Vector3(suspensionParent.camberAngle + suspensionParent.casterAngle * suspensionParent.steerAngle * suspensionParent.flippedSideFactor, -suspensionParent.toeAngle * suspensionParent.flippedSideFactor + suspensionParent.steerDegrees, 0);
		}
