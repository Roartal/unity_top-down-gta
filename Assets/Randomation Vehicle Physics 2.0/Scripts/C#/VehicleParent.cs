...
public class VehicleParent : MonoBehaviour
{
	...
	void FixedUpdate()
	{
        ...
        if(rb.velocity.magnitude > 1f && accelInput <= 0f)
        {
            rb.AddForce((-rb.velocity.normalized)* Mathf.Clamp((rb.velocity.magnitude/4f),0f,2f), ForceMode.Force);
        }
	}
					...
					if (Vector3.Dot(curCol.normal, col.relativeVelocity.normalized) > 0.2f && col.relativeVelocity.sqrMagnitude > 20)
					{
						...
						if (checkTow)
						{
							...
							if (sparks && playCrashSparks)
							{
								...
								//sparks.Emit((int)col.impulse.magnitude);
							}
						}
					}
				}
