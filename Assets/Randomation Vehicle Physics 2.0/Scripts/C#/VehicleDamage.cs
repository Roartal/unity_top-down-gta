...
public class VehicleDamage : MonoBehaviour
{
	...
	public Motor damagedMotor;
	public Transmission damagedTransmission;
		...
		if (hitTime == 0 && col.relativeVelocity.sqrMagnitude * damageFactor > 1 && strength < 1)
		{
			...
			foreach (ContactPoint curCol in col.contacts)
			{
				if (tr.InverseTransformPoint(curCol.point).y > collisionIgnoreHeight && GlobalControl.damageMaskStatic == (GlobalControl.damageMaskStatic | (1 << curCol.otherCollider.gameObject.layer)))
				{
					...
					if (curCol.otherCollider.GetComponent<HitElement> ()) {
						curCol.otherCollider.GetComponent<HitElement> ().Wake (col.impulse);
					}
					...
					if (vp.sparks && !sparkPlayed)
					{
						...
						vp.sparks.Emit((int)col.impulse.magnitude);
						//print ((int)col.impulse.magnitude);
						damageTaken += (int)col.impulse.magnitude;
						sparkPlayed = true;
					}
	...
	void DamageApplication(Vector3 damagePoint, Vector3 damageForce, float damageForceLimit, Vector3 surfaceNormal, ContactPoint colPoint, bool useContactPoint)
	{
		...
		Vector3 seamLocalPoint = new Vector3(0,0,0);
		Vector3 vertProjection = new Vector3(0,0,0);
		Vector3 translation = new Vector3(0,0,0);
		Vector3 clampedTranslation = new Vector3(0,0,0);
		Vector3 localPos;
		float vertDist = 0;
		float distClamp = 0;
		...
		if (damagedMotor)
		{
			damagePartFactor = colMag * surfaceDot * massFactor * Mathf.Min(clampedColMag * 0.01f, (clampedColMag * 0.001f) / Mathf.Pow(Vector3.Distance(damagedMotor.transform.position, damagePoint), clampedColMag));
			damagedMotor.health -= damagePartFactor * (1 - damagedMotor.strength);
		}

		//damageTaken += damagePartFactor;

		//Damage transmissions
		if (damagedTransmission)
		{
			damagePartFactor = colMag * surfaceDot * massFactor * Mathf.Min(clampedColMag * 0.01f, (clampedColMag * 0.001f) / Mathf.Pow(Vector3.Distance(damagedTransmission.transform.position, damagePoint), clampedColMag));
